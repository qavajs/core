...
const { expect } = require('@qavajs/validation');
...
When('I verify that memory loaded', async function() {
    expect(this.memory.getValue('$customValue')).toEqual('cjs');
});

When('I verify that memory is connected to qavajs world', async function() {
    expect(this.getValue('$customValue')).toEqual('cjs');
    expect(this.getValue('$additionalValue')).toEqual(12);
});

When('I verify that process env loaded', async function() {
    expect(process.env.CONFIG).toEqual('test-e2e/cjs/config.js');
...
When('Read memory {value} from cucumber type', async function(memoryValue) {
    expect(memoryValue.value()).toEqual('cjs');
});
